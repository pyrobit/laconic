#!/usr/bin/env python3
"""Benchmark Laconic vs normal Claude output token counts."""
...
def run_benchmarks(client, model, prompts, laconic_system, trials):
    results = []
    total = len(prompts)
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        laconic_medians = statistics.median(
            [t["output_tokens"] for t in entry["laconic"]]
        )
        savings = 1 - (laconic_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "laconic_median": int(laconic_medians),
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark Laconic vs normal Claude")
    ...
    results = run_benchmarks(client, args.model, prompts, laconic_system, args.trials)
