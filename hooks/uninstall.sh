#!/bin/bash
# laconic — uninstaller for the SessionStart + UserPromptSubmit hooks
# Removes: hook files in ~/.claude/hooks, settings.json entries, and the flag file
# Usage: bash hooks/uninstall.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/bruno335548975/laconic/main/hooks/uninstall.sh)
set -e

CLAUDE_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}"
HOOKS_DIR="$CLAUDE_DIR/hooks"
SETTINGS="$CLAUDE_DIR/settings.json"
FLAG_FILE="$CLAUDE_DIR/.laconic-active"

HOOK_FILES=("package.json" "laconic-config.js" "laconic-activate.js" "laconic-mode-tracker.js" "laconic-statusline.sh")

# Detect if laconic is installed as a plugin (check plugin cache)
PLUGIN_INSTALLED=0
if [ -d "$CLAUDE_DIR/plugins" ]; then
  if find "$CLAUDE_DIR/plugins" -path "*/laconic*" -name "plugin.json" -print -quit 2>/dev/null | grep -q .; then
    PLUGIN_INSTALLED=1
  fi
fi

if [ "$PLUGIN_INSTALLED" -eq 1 ]; then
  echo "Laconic appears to be installed as a Claude Code plugin."
  echo "To uninstall the plugin, run:"
  echo ""
  echo "  claude plugin disable laconic"
  echo ""
...
echo "Uninstalling laconic hooks..."
...
if [ -f "$SETTINGS" ]; then
  ...
  if ! command -v node >/dev/null 2>&1; then
    ...
    echo "         Remove the laconic SessionStart and UserPromptSubmit"
    ...
    LACONIC_SETTINGS="$SETTINGS" LACONIC_HOOKS_DIR="$HOOKS_DIR" node -e "
      ...
      const managedStatusLinePaths = [
        hooksDir + '/laconic-statusline.sh'
      ];
      ...
      // Remove statusLine if it references managed laconic scripts
...
echo ""
echo "Other agents:"
echo "  npx skills remove laconic    # Cursor, Windsurf, Cline, Copilot, etc."
echo "  claude plugin disable laconic  # Claude Code plugin"
echo "  gemini extensions uninstall laconic  # Gemini CLI"
