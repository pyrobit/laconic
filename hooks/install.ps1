# laconic — one-command hook installer for Claude Code (Windows PowerShell)
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: 'node' is required to install the laconic hooks (used to merge" -ForegroundColor Red
...
if (-not $Force) {
    ...
    if ($AllFilesPresent -and (Test-Path $Settings)) {
        try {
                ...
                foreach ($entry in $entries) {
                    if ($entry.hooks) {
                        foreach ($hookDef in $entry.hooks) {
                            if ($hookDef.command -and $hookDef.command -match 'laconic-(activate|mode-tracker)\.js') {
                                return $true
                            }
                        }
                    }
            ...
            $HooksWired = (& $hasLaconicHook "SessionStart") -and (& $hasLaconicHook "UserPromptSubmit")
    ...
    if ($AllFilesPresent -and $HooksWired -and $HasStatusLine) {
        Write-Host "Laconic hooks already installed in $HooksDir"
...
if ($Force -and (Test-Path (Join-Path $HooksDir "laconic-activate.js"))) {
    Write-Host "Reinstalling laconic hooks (-Force)..."
} else {
    Write-Host "Installing laconic hooks..."
}
...
const managedStatusLinePath = hooksDir + '/laconic-statusline.ps1';
...
Write-Host "  - SessionStart hook: auto-loads laconic rules every session"
